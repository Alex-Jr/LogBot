...
export default class MusicSubscription {
	...
	public constructor(voiceConnection: VoiceConnection, textChannel: TextBasedChannels) {
		...
		this.voiceConnection.on('stateChange', async (oldState, newState) => {
			if (newState.status === VoiceConnectionStatus.Disconnected) {
                await this.handleDisconnection(newState)
			} else if (newState.status === VoiceConnectionStatus.Destroyed) {
				await this.handleDestroyed()
			} else if (
				!this.readyLock &&
    ...
    private async handleDisconnection(newState: VoiceConnectionDisconnectedState) {
